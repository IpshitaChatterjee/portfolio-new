...
function ProjectVideo({ src, onClick }: ProjectVideoProps) {
...
export default function Personal() {
  ...
  const [pendingProjectLink, setPendingProjectLink] = useState('')

  const handleProjectClick = (project: any) => {
    const destination = project.blogLink
    ...
    if (project.requiresPassword) {
      setPendingProjectLink(destination)
  ...
  const handlePasswordSuccess = () => {
    router.push(pendingProjectLink)
  }
          ...
          {PROJECTS.map((project) => (
            <div key={project.name} className="space-y-2">
              <div className="relative rounded-2xl bg-zinc-50/40 p-1 ring-1 ring-zinc-200/50 ring-inset dark:bg-zinc-950/40 dark:ring-zinc-800/50">
                <ProjectVideo src={project.video} onClick={() => handleProjectClick(project)} />
              </div>
              <div className="px-1">
                <button
                  className="font-base group relative inline-block font-[450] text-zinc-900 dark:text-zinc-50 text-left"
